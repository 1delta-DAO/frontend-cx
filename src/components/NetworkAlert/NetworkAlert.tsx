...
import { LendingProtocolLogo } from 'components/ProtocolLogo'
...
const ContentWrapper = styled.div<{ chainId: NetworkAlertChains; darkMode: boolean; logoUrl: string, isColumn: boolean }>`
...
const LinkOutToBridgeFirst = styled(ExternalLink) <{ isColumn: boolean }>`
  align-items: center;
  ${({ isColumn }) =>
    isColumn ? `
...
const LinkOutToBridgeSecond = styled(ExternalLink) <{ isColumn: boolean }>`
  align-items: center;
  ${({ isColumn }) =>
    isColumn ? `
  ...
  text-decoration: none !important;
`
const Line = styled.div <{ isColumn: boolean }>`
...
const FAUCET_LINKS: { [protocol: string]: string } = {
  [LendingProtocol.AAVE]: 'https://app.aave.com/faucet/'
}
...
export function NetworkAlert({ isColumn = false }: AlertProps) {
  ...
  const { label, logoUrl, bridge, faucet } = getChainInfo(chainId)
