export const ERROR_MESSAGES: { [id: string]: string } = {
...
export const parseMessage = (error: any) => {
  const reason = error?.reason
  if (reason) {
    if (reason.includes("user rejected transaction")) return "User rejected the transaction"
    const message = ERROR_MESSAGES[reason.replace('execution reverted: ', '')]
    if (message)
      return message
  }
  const message = error?.message
  if (message) {
    if (message.includes("user rejected transaction")) return "User rejected the transaction"
    const messageParsed = ERROR_MESSAGES_COMP[message.replace('execution reverted: ', '')]
    if (messageParsed)
...
export const ERROR_MESSAGES_COMP: { [id: string]: string } = {
  'STF': 'Transfer failed'
}


export const parseMessageCompound = (error: any) => {
  const reason = error?.reason
  if (reason) {
    if (reason.includes("user rejected transaction")) return "User rejected the transaction"
    const message = ERROR_MESSAGES_COMP[reason.replace('execution reverted: ', '')]
    if (message)
      return message
    else return reason
  }
